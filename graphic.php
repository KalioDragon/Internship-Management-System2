...
echo "Failed to connect to MySQL: " . mysqli_connect_error();
        ...
        $info=mysqli_query($con,"SELECT C_name,c_info,c_criteria,location FROM Company WHERE A_id=5");

	
	if(isset($_POST["Apply"]))
	{
	$cname=$_POST['Apply'];
	$aid=mysqli_query($con,"SELECT A_id from AreaOI WHERE Field_name='Graphic Design'");		
	while($row = mysqli_fetch_array($aid)){
	if($row['A_id'])
{
		$aid=$row['A_id'];
		session_start();
		$_SESSION['aidgraphic']=$aid;
		$_SESSION['cnamegraphic']=$cname;
